...
<div class="min-h-full">
    <div class=" mx-auto">
        <form method="post" action=" {{ route('admin.song.save.updated',['id' => $song->id]) }} " class="space-y-8 divide-y divide-gray-200" enctype="multipart/form-data">
            @csrf
            <div class="grid md:grid-cols-2 ">
                <div class="p-4">
                    <div class="space-y-6 sm:space-y-5">
                        <div class="">
                            ...
                            <x-text-input id="title" class="block mt-1 w-full" type="text" name="title" :value="old('title', $song->title)" required autofocus autocomplete="title" />
                            <x-input-error :messages="$errors->get('title')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="release_date" :value="__('release date')" />
                            <x-text-input id="release_date" :value="old('release_date',$song->release_date)" class="block mt-1 w-full" type="date" name="release_date" required />
                        ...
                        <div class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                            ...
                            <div class="mt-1 sm:mt-0 sm:col-span-2">
                                <textarea id="lyrics" name="lyrics" rows="3" class="max-w-lg shadow-sm block w-full focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm border border-gray-300 rounded-md">
                            ...
                            <div class="mt-1 sm:mt-0 sm:col-span-2">
                                <div class="max-w-lg flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md">
                                    <div class="space-y-1 text-center">
                                        <img class=" max-h-10" src="{{$song->cover_url}}" alt="" srcset="">
                                        <div class="flex text-sm text-gray-600 justify-center">
                                            <label for="cover" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                                                ...
                                                <x-text-input id="cover" class="sr-only" type="file" name="cover" :value="old('cover')" autofocus  />
                                            </label>
                ...
                <div class="p-4 ">
                    <div class="space-y-6 sm:space-y-5">
                        ...
                        <livewire:components.select name="artist" label="Artist" :selected="$artists"/>
                        <livewire:components.select name="band" label="band" :selected="$bands"/>
                        <livewire:components.select name="genre" label="genre" :selected="$genres"/>
                        <x-input-error :messages="$errors->get('genre')" class=" font-light text-sm" />
                        <livewire:components.select name="writer" label="writer" :selected="$writers"/>
                        <livewire:components.select name="language" label="language" :selected="$languages"/>
            ...
            <div class="pt-5">
                ...
                <a href="/admin/songs/list" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Cancel</a href="admin/songs/list">
