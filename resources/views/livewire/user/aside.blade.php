<div>
    ...
    <div x-data="musicPlayer()" x-init="init(); $watch(() => $refs.playlistData.value, (newValue) => { tracks = JSON.parse(newValue); trackIndex = 0; currentTrack = tracks.songs[trackIndex]; currentplaylist.name = tracks.name; });" x-show="$wire.playing_playlist">

        <input type="hidden" x-ref="playlistData" wire:model="playing_playlist" value="@json($playing_playlist)">
        ...
        <script>
            console.log('loaded')
            const playing_playlist = @json($playing_playlist);
            ...
            function musicPlayer() {
                return {
                    ...
                    tracks: playing_playlist,
                    trackIndex: 0,

                    init() {
                        this.currentTrack = this.tracks.songs[this.trackIndex];
                        this.currentplaylist.name = this.tracks.name;
                    },
                    ...
                    playPrevious() {
                        this.trackIndex = (this.trackIndex - 1 + this.tracks.songs.length) % this.tracks.songs.length;
                        this.currentTrack = this.tracks.songs[this.trackIndex];
                        this.isPlaying = true;
                        this.$refs.audioPlayer.play();
                    },
                    playNext() {
                        this.trackIndex = (this.trackIndex + 1) % this.tracks.songs.length;
                        this.currentTrack = this.tracks.songs[this.trackIndex];
                        this.isPlaying = true;
                        this.$refs.audioPlayer.play();
                    },
    ...
    <button wire:click='setPlayingPlaylist({{ $playlist->id }})'>Set as playing playlist</button>

    <div id="accordion-collapse-{{$playlist->id}}" data-accordion="collapse">
        <h2 id="accordion-collapse-heading-{{$playlist}}">
